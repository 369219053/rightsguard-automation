...
22. 处理系统录屏权限弹窗 ✅
  ...
  ├─ 22.6 点击"整个屏幕"选项
  ├─ 22.7 按钮变为"立即开始"
  └─ 22.8 下次事件触发时点击"立即开始" → 录屏开始 ✅
  ↓
23. 🎯 点击"开始取证"按钮 (V2.4新增)
  ├─ 设置权利卫士取证阶段标志位
  └─ 权利卫士自动跳转到抖音
  ↓
24. 🆕 权利卫士取证阶段 - 抖音自动化 (V2.4新增) ⭐️
  ├─ 24.1 检测到抖音"我"页面
  ├─ 24.2 ⏱️ 随机延迟1-2秒(模拟真人操作)
  ├─ 24.3 点击右上角"更多"按钮(三条杠)
  ├─ 24.4 ⏱️ 随机延迟1-2秒
  ├─ 24.5 点击"设置"按钮
  └─ 24.6 进入抖音设置页面
  ↓
...
**两次打开抖音的区别**:
1. **第一次** (V2.3) - 我们的APP通过WebView打开侵权视频 → 观看5秒 → 返回首页 → 点击"我" → 打开权利卫士
2. **第二次** (V2.4) - 权利卫士点击"开始取证" → 权利卫士自动跳转到抖音 → **需要自动化操作**

**V2.4新增: 权利卫士取证阶段自动化**
- 🎯 **阶段标识** - 点击"开始取证"后设置标志位,进入权利卫士取证阶段
- 📱 **监听抖音事件** - 在取证阶段监听抖音包名的窗口事件
- ⏱️ **随机延迟** - 每步操作前随机延迟1-2秒,模拟真人操作
- 🔍 **智能点击** - 优先通过文本查找,失败则使用坐标点击
