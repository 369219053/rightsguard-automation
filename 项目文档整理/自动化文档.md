...
15. 等待2秒(界面加载)
  ↓
16. 填充备注内容到输入框 (只填充"原创名称-抖音:侵权人名称")
  ↓
17. 点击"开始录屏取证"按钮
  ↓
18. 系统录屏权限弹窗出现 ✅
...
19. 处理系统录屏权限弹窗 ✅
  ...
  ├─ 19.6 点击"整个屏幕"选项
  ├─ 19.7 按钮变为"立即开始"
  └─ 19.8 下次事件触发时点击"立即开始" → 录屏开始 ✅
  ↓
20. 录屏成功开始 ✅
  ↓
21. 🎬 进入录屏阶段 (所有操作需要随机延迟)
  ↓
22. 检测到抖音口令打开页面
  ↓
23. ⏱️ 随机延迟 1.5s-3s (模拟真人操作)
  ↓
24. 点击"打开看看"按钮 ✅
  ↓
25. 进入抖音视频播放页面
...
#### 0. 🎯 随机延迟机制 (V1.9新增) ⭐️ 重要!

**功能**: 模拟真人操作,避免被检测为机器人

**为什么需要随机延迟?**
- 📹 **录屏阶段的操作会被记录** - 从点击"打开看看"开始,所有操作都在录屏中
- 🤖 **操作过快会显得不自然** - 机器人操作速度过快,容易被识别
- ⏱️ **操作过慢会浪费时间** - 录屏文件过大,影响效率
- 🎭 **随机延迟模拟真人** - 每次操作间隔不同,更加真实
...
**日志输出**:
```
⏱️ 随机延迟 2.34 秒 (模拟真人操作)
```

**重要提示**:
- ⚠️ **录屏前的操作不需要延迟** (如打开权利卫士、填写备注等)
- ⚠️ **录屏后的所有操作都需要延迟** (从点击"打开看看"开始)
- ⚠️ **延迟时间必须随机** (不能固定,否则仍然像机器人)
...
1. 提取原创名称: "郭惠文"
2. 提取侵权人: "文文工艺品"
3. 智能提取第一个URL: "https://v.douyin.com/xxx"
4. 智能提取第二个URL: "https://www.douyin.com/yyy"
  ↓
输出备注: "郭惠文-抖音:文文工艺品"
```
...
// "打开看看"按钮ID
private static final String DOUYIN_OPEN_BUTTON_ID = "com.ss.android.ugc.aweme:id/tnn";
...
2. 判断是否为口令打开页面
  ↓
3. 查找"打开看看"按钮
  ├─ 方法1: 通过ID查找 (com.ss.android.ugc.aweme:id/tnn)
  └─ 方法2: 通过文本查找 ("打开看看")
  ↓
4. ⏱️ 随机延迟 1.5s-3s (模拟真人操作)
  ↓
5. 点击"打开看看"按钮
  ↓
6. 进入视频播放页面
...
private void handleDouyinOpenDialog() {
    ...
    try {
        ...
        if (buttonNodes != null && !buttonNodes.isEmpty()) {
            ...
            new Thread(() -> {
                try {
                    ...
                    // 点击按钮
                    boolean clicked = buttonNode.performAction(ACTION_CLICK);

                    if (clicked) {
                        Log.d(TAG, "✅ 成功点击'打开看看'按钮");
                        hasClickedDouyinOpen = true;
                    }

                    buttonNode.recycle();
                } catch (Exception e) {
                    Log.e(TAG, "点击异常: " + e.getMessage());
                }
            }).start();
        ...
        Log.e(TAG, "处理抖音页面异常: " + e.getMessage());
    }
}
```
...
**注意事项**:
- ⚠️ **此操作在录屏阶段** - 必须使用随机延迟
- ⚠️ **操作会被记录** - 延迟时间影响录屏真实性
- ⚠️ **不能过快或过慢** - 1.5s-3s是最佳范围
