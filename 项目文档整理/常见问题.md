...
8. [坐标点击问题](#坐标点击问题)
9. [抖音启动广告问题](#抖音启动广告问题)
10. [带货数据/创作灵感/带货达人显示问题](#带货数据创作灵感带货达人显示问题)
...
### Q6.1: 带货数据滚动时一直滑不停，无法点击"30天"？(V4.4已解决)

**问题描述**:
下滑到带货数据区域时，OCR识别不到"带货数据"，导致一直滚动最终滑出页面。

**根本原因**:
Google ML Kit OCR在某些字体渲染下会把"数"字误识别为"教"字：
- `"带货数据"` → OCR读成 `"带货教据"` 或 `"带货数"`（截断）
- 精确匹配"带货数据"必然失败

**解决方案 (V4.4 - 模糊候选词双组检测)**:

新增 `findDualTextPositions()` 方法，使用候选词列表代替精确匹配：

```java
// 带货数据候选组（覆盖"数"→"教"的各种误读）
String[] group1 = {"带货数据", "带货教据", "带货数"};
// 受众数据候选组
String[] group2 = {"受众数据", "受众教据", "受众数"};

OcrHelper.findDualTextPositions(bitmap, group1, group2, callback);
```

**停止条件**: 两组候选词**同时**命中才停止下滑，确保带货图表和受众图表都在屏幕内，两个"30天"按钮均可见。

**30天按钮选取**: 取所有"30天"中与"带货数据"Y坐标差值最小的那个（上面那个），避免误点受众数据旁的"30天"。

**日志示例**:
```
🎯 [双组] 命中组1: '带货教据' Y=680
🎯 [双组] 命中组2: '受众数据' Y=1250
✅ 第3次下滑：'带货教据'(Y=680) + '受众数据'(Y=1250) 同时可见，停止下滑！
```

---

### Q6.2: 创作灵感侵权视频点击后打开的不是侵权视频？(V4.4已解决)

**问题描述**:
通过封面Key匹配到侵权视频节点，但点击后打开的不是正确的视频，或点击无效。

**根本原因**:
扫描完无障碍树后调用 `root.recycle()` 释放资源，但代码仍持有节点引用。调用 `recycle()` 后节点的 `getBoundsInScreen()` 返回空Rect（0,0,0,0），导致坐标失效。

**解决方案 (V4.4 - 坐标先存后recycle)**:
...
if (bounds.width() > 50 && bounds.height() > 50 && bounds.bottom <= 2400) {
    matchedBounds.add(new Rect(bounds)); // 保存Rect副本
}
root.recycle(); // 现在可以安全recycle
// 后续直接用保存的Rect坐标点击
clickByCoordinates(rect.centerX(), rect.centerY());
```

---

### Q6.3: 侵权视频取证截图没有账号验证，怎么确认视频归属？(V4.4已解决)

**功能说明 (V4.4新增)**:
点击侵权视频后，自动执行以下验证和取证流程：

1️⃣ **账号名称OCR验证** - 截图当前屏幕，OCR检测是否包含 `infringerName`（侵权人账号名）
- 验证通过 → 继续截图取证
- 未找到 → 跳过此视频，按返回键

2️⃣ **三点时间截图** - 按视频时长的 25% / 50% / 75% 时间点各截图1张

3️⃣ **等待视频播完** - 基于真实时钟计算剩余时间，视频播完后按返回键

4️⃣ **多视频循环** - 当前页面有N个匹配视频，逐个处理，每个都执行上述3步

**截图文件名格式**:
```
购物车取证_创作灵感_侵权视频1_截图1_25pct_20260320_162500.png
购物车取证_创作灵感_侵权视频1_截图2_50pct_20260320_162530.png
购物车取证_创作灵感_侵权视频1_截图3_75pct_20260320_162600.png
```
...
### Q6.4: 图形滑块验证码滑动距离始终为0，验证无法通过？(V5.3已解决)

**问题描述**:

日志显示本地模板匹配完成，但最优X坐标始终为0px，相关得分高达0.993，滑块没有实际移动，验证每次失败。

```
✅ 本地模板匹配完成：最优X=0px，相关得分=0.993   ← 异常：每次都0
🖐️ 本地匹配滑动距离=0px，目标x=222，模拟人手滑动...
```

**根本原因**:

NCC（归一化互相关）算法的搜索起点设为 `offsetX=0`，而碎片图（162×162px）在背景图裁剪坐标系中的起始位置正好是 X=0（碎片实际坐标 `[141,1060]→[303,1222]`，背景图从 `[141,895]` 开始裁剪，两者左边缘对齐）。

因此算法在 X=0 处与碎片自身完美重叠，相关系数高达0.993，误把**碎片的当前位置当成了缺口位置**。

**修复方案 (V5.3)**:

将搜索循环起点从 `offsetX=0` 改为 `offsetX=fgW`（碎片宽度=162px），强制跳过碎片初始所在范围，只在碎片右侧寻找真正的缺口：

```java
// ❌ 修复前：从0开始，命中碎片自身
for (int offsetX = 0; offsetX < maxX; offsetX++)

// ✅ 修复后：从fgW(162px)开始，跳过碎片初始位置
...
```
✅ 本地模板匹配完成：最优X=187px，相关得分=0.721   ← 正常：找到真实缺口
...
### Q13.3: 点击"我的订单"后出现悬浮小广告，导致无法点击"更多"？（V5.6已解决）

**问题描述**:

进入"我的订单"页面后，有时界面会浮现一个悬浮小广告（独立小窗口），遮挡"更多（三个点）"按钮，导致点击打到错误位置或无响应。

**根本原因**:

抖音"我的订单"页偶发性弹出一个独立的小浮窗广告（通过Accessibility Window树可见），该窗口处于**聚焦状态**，坐标点击会先落在它上面而非底层的"更多"按钮。
...
|------|----|
| 窗口类型 | 独立悬浮窗 |
| 窗口宽度 | ≈60px（远小于屏幕宽度1080px） |
| 聚焦状态 | 活动=是，聚焦=是 |

**解决方案（V5.6 — `dismissSmallFloatingAdIfPresent()`）**:

新增方法通过**宽度 < 200px** 识别悬浮小广告窗口，在点击"更多"之前自动检测并关闭：

```
点击"更多"（三个点）前：
    → dismissSmallFloatingAdIfPresent()
        ├── 遍历所有Accessibility窗口
        ├── 找到宽度 < 200px 的窗口 → 发送BACK键关闭
        └── 未找到 → 直接跳过
    → 正常点击"更多" ✅
```
...
### Q9.1: 打开抖音时弹出广告页面，自动化没有点击"跳过广告"？

**问题描述**:
- 首次打开抖音时偶尔弹出全屏启动广告页面
- 广告页面不能点击正文区域（会跳转到第三方App）
- 只能点击右上角"跳过广告"按钮或等倒计时结束

**根因（V4.5修复前）**:
```
测试模式路径（testModeThread）：
switchToDouyin() → 等待抖音到前台 → 直接执行Step2（点导航栏）
                                     ↑ 广告检测从未在此调用！

主流程路径（startDouyinAutomation）：
switchToDouyin() → skipDouyinSplashAdIfPresent() → 继续
                  ↑ 只有这里有广告检测
```

**V4.5修复方案**: 在测试模式确认抖音到前台后、Step2之前插入广告检测调用：
```java
// 确认抖音到前台（已有逻辑）
logD("✅ 第N秒检测到抖音已到前台");
...
**广告检测逻辑**（`skipDouyinSplashAdIfPresent()`）:

| 项目 | 详情 |
|------|------|
| 最大等待 | 10秒 |
| 轮询间隔 | 每500ms一次 |
| 无广告提前退出 | 3秒内未检测到广告直接返回 |
| 检测策略A | ID精确匹配 `com.ss.android.ugc.aweme:id/0m4` |
| 检测策略B | desc含"跳过广告"（防ID随版本变更） |
| 点击策略 | 优先无障碍API；失败则坐标兜底（自动计算按钮中心） |
| 点击后 | 等待3秒让广告页完全消失 |

**日志标识**:
- `🔍 [测试模式] 检测抖音启动广告...` → 检测已触发
- `✅ [广告检测] 发现'跳过广告'按钮(ID:0m4)，准备点击...` → 有广告时
- `✅ [广告检测] 等待XXXms，未检测到启动广告，正常启动` → 无广告时

---

### Q9.2: 广告检测失败，点击了广告正文区域？

**原因**: 广告正文区域点击会跳转到第三方App，只允许点击右上角"跳过广告"按钮。

**防护措施**:
- `skipDouyinSplashAdIfPresent()` 只点击检测到的跳过按钮节点，**不会盲目点击任意坐标**
- 跳过按钮ID `0m4` 是抖音广告的固定组件，不含任何可跳转逻辑
- 若无障碍API点击失败，坐标兜底也是计算 `0m4` 节点的中心坐标（右上角），不会触及广告正文

---

## 🛒 带货数据/创作灵感/带货达人显示问题

### Q10.1: 带货数据板块点击"热度"Tab后未完整显示？

**原因**: 抖音选品带货页面的带货数据图表在首次进入时存在懒加载问题，直接点击顶部"热度"Tab有时无法触发完整渲染，页面会显示空白或只渲染部分内容。

**解决方案（V4.7起）**: 改回OCR滚动策略，不依赖Tab点击：
- 下滑页面（步长350px，最多20次），OCR同时检测"带货数据"和"受众数据"
- 两组文字**同时**出现 = 带货数据图表已完整渲染（图表底部的受众数据标题进入可视区域）
- 停止下滑后立即进行30天→90天时间筛选并截图，无需额外等待

**日志判断**:
- `✅ 第N次下滑：'带货数据'(Y=xxx) + '受众数据'(Y=xxx) 同时可见，停止下滑！` → 正常
- `❌ 下滑20次后仍未检测到'带货数据'` → 页面加载异常或商品无带货数据

---

### Q10.2: 创作灵感板块内容显示不正确？

**原因**: 顶部导航Tab中，"评价"是评论区，"内容"才是创作灵感视频列表。V4.7之前误点了"评价"。

**解决方案（V4.7起）**: 统一点击**"内容"Tab**（坐标 736,277），确保显示侵权视频列表。

**Tab坐标对照表（创作灵感页面顶部导航栏）**:
| Tab标签 | Bounds | 中心坐标 | 用途 |
|---|---|---|---|
| 热度 | [45,219]→[246,336] | (145, 277) | ⚠️ Dump按钮覆盖区域 |
| 受众 | [243,219]→[444,336] | (343, 277) | - |
| 评价 | [441,219]→[639,336] | (540, 277) | 评论区（不用） |
| **内容** | [636,219]→[837,336] | **(736, 277)** | ✅ 创作灵感视频列表 |
| **达人** | [834,219]→[1035,336] | **(934, 277)** | ✅ 带货达人列表 |

---

### Q10.3: 悬浮窗的Dump按钮遮挡了"热度"Tab，导致误点击？

**原因**: Dump按钮位置 `[100,214]→[280,394]` 与"热度"Tab坐标 `(145,277)` 完全重叠，坐标点击会触发Dump截图。

**解决方案**:
- `clickNavTab()` 方法优先使用**无障碍API**通过 `tabBarIndex-0` 节点ID点击，完全绕开Dump按钮覆盖
- 若无障碍API未命中则坐标兜底（此时Dump按钮若未显示则正常）
- 带货数据板块已恢复OCR滚动策略，不再依赖点击"热度"Tab

---

### Q10.4: 悬浮窗无法拖动怎么办？

**原因（已修复）**: 旧版将拖动监听挂在整个 `layoutFull` 上，子按钮会消费 `ACTION_DOWN` 事件，导致父布局无法收到 `MOVE/UP`。

**解决方案（V4.6+）**: 拖动监听挂在 `view_drag_handle`（专用空View），该View无任何子控件，`ACTION_DOWN` 正确返回 `true`，拖动完全生效。

**操作方式**: 长按悬浮窗**顶部拖动条区域**（非按钮区）即可拖动。

---

### Q10.5: 最小化后点击悬浮窗图标直接触发Dump截图？

**原因（已修复）**: 旧版最小化按钮的 `onClick` 监听直接调用 `dumpCurrentUI()`，点击恢复操作无法拦截。

**解决方案（V4.6+）**:
- 最小化图标改为 `"⬆"` 标识
- 通过 `OnTouchListener` 拦截触摸事件：**短按** = 恢复悬浮窗，**拖动** = 移动图标位置
- Dump功能只能通过展开后的Dump按钮触发

---

### Q10.6: 带货达人板块被跳过，日志提示"未设置侵权人名称"？（V5.2修复）

**症状**: 日志出现以下信息，带货达人流程被完全跳过：
```
⚠️ [带货达人] 未设置侵权人名称，截图已保存，跳过达人点击检测
```
...
旧版数据流存在两次解析的脆弱链路：
```
parseEvidenceInfo() → 正确提取 infringerName
  → 但只打包成 remark 字符串 "邸元元-抖音:家加鲜餐饮具旗舰店"
  → 传给 setRemark()
  → setRemark() 需要再次解析 remark → 任何格式差异均会导致解析失败
  → infringerName = "" → 带货达人流程被跳过
```

**常见触发场景**:
1. **全角冒号**：用中文输入法输入 `抖音：` 时，系统自动将 `:` 转为全角 `：`（U+FF1A），导致 `-抖音:` 匹配失败
2. **格式细微差异**：remark 字符串与解析期望不完全一致

**解决方案（V5.2起）**: 已彻底重构数据传递架构：
- `parseEvidenceInfo()` 解析后，**直接**将结果存入 `ParseResult.infringerName`
- `MainActivity` 三个模式（正常/测试/带货测试）均调用 `service.setInfringerName()` 独立setter
- `AutomationAccessibilityService` 的 `setInfringerName()` 直接赋值，无任何二次解析
- 输入时同时标准化全角→半角冒号，两重保险

**验证方法**: 日志中看到以下诊断行，确认 `infringerName` 非空即表示正常：
```
🔍 [带货达人] 诊断 → infringerName='家加鲜餐饮具旗舰店' originalName='邸元元' remark='...'
```
...
**最后更新**: 2026-03-30 (V5.6 新增Q13.3 我的订单悬浮小广告问题)
