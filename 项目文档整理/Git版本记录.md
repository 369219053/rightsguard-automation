...
### V5.6 (2026-03-30) 🐛 第二张截图竞态修复 + 我的订单悬浮广告检测 + Ghost Click根治

**✨ 核心更新 - 三项稳定性修复 + 店铺标签多变体识别**

#### 🐛 Bug修复

**1. 第二张截图竞态条件修复（权利卫士_勾选抖音）**

- **根因**：正常模式中，侵权链接预先将抖音拉至前台。点击抖音容器后的1-3秒随机延迟期间，抖音会抢占前台，导致截图捕获的是抖音首页而非权利卫士的"勾选抖音"界面。
- **修复**：使用 `CountDownLatch` 将截图移至随机延迟**之前**同步执行。截图成功后才开始随机延迟，彻底消除竞态窗口。
- **简化**：原有两张截图（权利卫士_勾选抖音 + 应用验真）精简为**仅一张**（权利卫士_勾选抖音），流程更简洁，取证截图内容更准确。

```
点击抖音容器成功
    ↓ (后台线程，CountDownLatch同步)
📸 截图：权利卫士_勾选抖音  ← 此时仍在权利卫士界面，截图必然正确
    ↓
⏳ 随机延迟 1-3 秒（此时抖音可能回到前台，已无影响）
    ↓
🖱️ 点击"立即验证"
```
...
- **根因**：点击"我的订单"后，界面有时浮现一个小广告（独立小窗口，宽度约60px），导致后续点击"更多"（三个点）失败或打到错误位置。
- **修复**：新增 `dismissSmallFloatingAdIfPresent()` 方法，通过检测**宽度 < 200px** 的独立窗口识别悬浮小广告。点击"更多"之前自动检测并关闭，无广告则直接跳过。

**3. 停止录屏后 Ghost Clicks 根治**

- **根因**：带货达人子流程在 `checkAndCaptureShoppingCart()` 内部将 `isRunning = false`，但返回后父流程未检查该标志，继续执行收尾逻辑，造成重复点击。
- **修复**：在 `checkAndCaptureShoppingCart()` 返回后立即检查 `if (!isRunning) return;`，彻底阻断重复执行路径。

**4. 创作灵感作者主页店铺标签多变体识别**

- **问题**：店铺标签名称在不同账号/版本下有多种表现形式，原代码仅识别"店铺账号"。
- **修复**：`captureAuthorProfileEvidence()` 现在按优先级依次搜索4种变体：
  ```
  店铺账号 → 店铺授权号 → 店员 → 职人
  ```
  找到任意一种即停止搜索并点击。
...
- **移除**：验证测试、带货测试、销量测试三个测试按钮
- **新增**：「📸 第二张截图测试」按钮，完整复用正常模式流程（打开侵权链接→权利卫士→抖音→点击"我"），在点击"我"后停止，专门用于排查截图时序问题

#### 📁 修改文件

| 文件 | 修改内容 |
|------|---------|
| `AutomationAccessibilityService.java` | `handleDouyinVerification()` 使用CountDownLatch同步截图；移除第二张截图；新增 `dismissSmallFloatingAdIfPresent()`；`captureAuthorProfileEvidence()` 支持4种标签变体；`checkAndCaptureShoppingCart()` 返回后增加isRunning检查；新增 `startScreenshotTestMode()` |
| `MainActivity.java` | 移除3个旧测试按钮字段/方法；新增 `startScreenshotTestMode()` |
| `activity_main.xml` | 移除3个测试按钮；新增"📸 第二张截图测试"按钮 |
...
```
[11:05:56] 坐标点击成功: (313, 931)       ← 点击执行了
[11:05:58] 📝 文本块: 白近7日动态v        ← 页面默认是"近7日"不是"近30日"
[11:05:58] ❌ 未找到目标文字: 近90日       ← 弹窗根本没打开
```

- 带货达人时间筛选按钮**默认显示"近7日"**，而代码写死坐标 `(313,931)` 是按"近30日"测量的
- 坐标点不到按钮，弹窗未打开，OCR自然找不到"近90日"
- 点击"近90日"后缺少点击"确定"步骤，弹窗无法关闭

#### ✅ 修复内容

**文件**: `app/src/main/java/com/rightsguard/automation/AutomationAccessibilityService.java`
**方法**: `checkLeadingCreators()` — Step 0.5 时间筛选逻辑

**改动1：筛选按钮改用OCR动态定位（弃用硬编码坐标）**

```java
// ❌ 旧：写死坐标，仅适配"近30日"为默认状态
clickByCoordinates(313, 931);

// ✅ 新：OCR多关键词搜索，兼容任意初始状态
filterOcr.findAnyTextPosition(bitmap,
    new String[]{"近30日", "近7日", "30天", "7天"},
    new OcrHelper.OcrAnyCallback() { ... });
// 找到后取所有匹配中Y最小的（最靠上的筛选按钮）点击
```

**改动2：点击"近90日"后新增点击"确定"关闭弹窗**

```java
// ✅ 点击"近90日"后等500ms，OCR查找"确定"按钮并点击
clickByCoordinates(nd90Pos[0], nd90Pos[1]);   // 点击近90日
Thread.sleep(500);
// OCR找"确定" → 点击 → 等待1500ms弹窗关闭
// 找不到则按返回键兜底关闭弹窗
```
...
🛒 带货测试按钮
    ① 切换到抖音 → 等待最多8秒
    ② 点击"达人"标签(934,277) → 等待1500ms
    ③ checkLeadingCreators()
        [Step 0]   页面加载检测 + 空白页恢复
        [Step 0.5] OCR找时间筛选按钮（近7日/近30日均兼容）
                   → 点击打开弹窗 → OCR找"近90日"点击
                   → OCR找"确定"点击关闭弹窗
        [Step 1]   📸 截图带货达人整体区域
        [Step 2]   查找侵权账号（需设置infringerName）
```
